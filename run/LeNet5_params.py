...
params_single = [(30, 1e-3, 10)]  # comparison group params
...
settings_dataset = [(_param, dataset_settings_all) for _param in params_single]
...
PLOTS_SHOW = True  # True=show, False=NOT show
PLOTS_SAVE_DPI = 200
...
(bs, lr, ep), d_set = settings_dataset[0]
...
ax.set_xticklabels(DATASET_LABELS, rotation=30)
ax.set_yticklabels(DATASET_LABELS, rotation=30)
...
plt.close("all")
fig = plt.figure()
((_, lr, ep), d_set) = settings_param[0]
...
plt.title("LearningRate=%.5f, Epoch=%d, Train&Test=%s" % (lr, ep, d_set[0]),
          fontsize=9)
...
plt.close("all")
fig = plt.figure()
((bs, lr, _), d_set) = settings_param[0]
d_set = d_set[0]  # from [(dataset, dataset)] to (dataset,dataset)
test_acc_ep = []
...
#           fontsize=9)
plt.title("BatchSize=%d, LearningRate=%.5f, Train&Test=%s" % (bs, lr, d_set[0]),
          fontsize=9)
...
((bs, _, ep), d_set) = settings_param[0]
d_set = d_set[0]  # from [(dataset, dataset)] to (dataset,dataset)
test_acc_lr = []
train_loss_ep__lrs = []
train_acc_ep__lrs = []
...
plt.title("BatchSize=%d, Epoch=%d, Train&Test=%s" % (bs, ep, d_set[0]),
          fontsize=9)
